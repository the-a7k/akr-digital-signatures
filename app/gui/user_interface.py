...
def frame_clean(frame):
    # Clean a frame from previous messages
...
def handle_file_signage(user, output_frame, file_path_for_signage):
    # Handle choice of correct signage algorithms
    file_extension = extract_file_extension(file_path_for_signage)
    new_file_path = file_add_signed_suffix(file_path_for_signage)
    frame_clean(output_frame)
    ...
    match(file_extension):
        case ".pdf":
            ...
            ttk.Label(output_frame, text="PDF has been signed successfully! Location:" , font=("Consolas", 12, "bold"), bootstyle='success').pack(pady=10)
            ttk.Label(output_frame, text=new_file_path , font=("Consolas", 8, "bold"), bootstyle='success').pack(pady=10)
        case ".png":
            ...
            ttk.Label(output_frame, text="PNG has been signed successfully! Location:" , font=("Consolas", 12, "bold"), bootstyle='success').pack(pady=10)
            ttk.Label(output_frame, text=new_file_path , font=("Consolas", 8, "bold"), bootstyle='success').pack(pady=10)
        case ".mp3":
            ...
            ttk.Label(output_frame, text="MP3 has been signed successfully! Location:" , font=("Consolas", 12, "bold"), bootstyle='success').pack(pady=10)
            ttk.Label(output_frame, text=new_file_path , font=("Consolas", 8, "bold"), bootstyle='success').pack(pady=10)
        case _:
            new_file_path = file_add_signed_suffix(file_path_for_signage, const.FILE_UNKNOWN_SIGNAGE_FILETYPE_SUFFIX)
            file = FileSignature(user.private_key, file_path_for_signage)
            file.create_signature_file(new_file_path)

            ttk.Label(output_frame, text="File signature has been created successfully! Location:" , font=("Consolas", 12, "bold"), bootstyle='success').pack(pady=10)
            ttk.Label(output_frame, text=new_file_path , font=("Consolas", 8, "bold"), bootstyle='success').pack(pady=10)

def handle_file_verification(user, output_frame, file_path_to_verify, file_path_to_verify_signature):
    # Handle choice of correct verification algorithms
    file_extension = extract_file_extension(file_path_to_verify)
    frame_clean(output_frame)
        ...
        case ".pdf":
            try:
                ...
                if new_pdf.compare_signature(signature):
                    ttk.Label(output_frame, text="PDF signature has been validated successfully!" , font=("Consolas", 12, "bold"), bootstyle='success').pack(pady=10)
                else:
                    ttk.Label(output_frame, text="PDF signature is incorrect! File has been changed!" , font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
            except Exception as e:
                    ttk.Label(output_frame, text="PDF has been corrputed! Unable to validate the file!" , font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
        ...
        case ".png":
            try:
                ...
                if new_png.compare_signature(signature):
                    ttk.Label(output_frame, text="PNG signature has been validated successfully!" , font=("Consolas", 12, "bold"), bootstyle='success').pack(pady=10)
                else:
                    ttk.Label(output_frame, text="PNG signature is incorrect! File has been changed!" , font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
            except Exception as e:
                    ttk.Label(output_frame, text="PNG has been corrputed! Unable to validate the file!" , font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
        ...
        case ".mp3":
            try:
                ...
                if new_mp3.compare_signature(signature):
                    ttk.Label(output_frame, text="MP3 signature has been validated successfully!" , font=("Consolas", 12, "bold"), bootstyle='success').pack(pady=10)
                else:
                    ttk.Label(output_frame, text="MP3 signature is incorrect! File has been changed!" , font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
            except Exception as e:
                    ttk.Label(output_frame, text="MP3 has been corrputed! Unable to validate the file!" , font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)

        case _:
            if path_valid(file_path_to_verify_signature) is False or len(file_path_to_verify_signature) == 0:
                ttk.Label(output_frame, text="Signature file not loaded!" , font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
                return
            try:
                ...
                if file.compare_signature(signature):
                    ttk.Label(output_frame, text="File signature has been validated successfully!" , font=("Consolas", 12, "bold"), bootstyle='success').pack(pady=10)
                else:
                    ttk.Label(output_frame, text="File signature is incorrect! File has been changed!" , font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
            except Exception as e:
                    ttk.Label(output_frame, text="Signature file has been corrupted! Unable to validate the file!" , font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)

def signature_on_submit(cert_auth, output_frame, username_entry, password_entry, file_to_be_signed_entry, file_to_be_verified, file_with_separate_signature):
    # App on-submit handler
    ...
    if file_user_password == "":
        frame_clean(output_frame)
        ttk.Label(output_frame, text="Username or password is missing!", font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
        return
    ...
    try:
        if (
            not user.file_load_certificate(user_path, file_user_name + const.USER_CERT_SUFFIX)
            or not user.file_load_private_key(user_path, file_user_name + const.USER_KEY_SUFFIX, file_user_password)
        ):
                ...
                ttk.Label(output_frame, text="Certificate and private key created successfully!", font=("Consolas", 12, "bold"), bootstyle='success').pack(pady=10)
        else:
            ttk.Label(output_frame, text="Certificate and private key loaded successfully!", font=("Consolas", 12, "bold"), bootstyle='success').pack(pady=10)
        
        if path_valid(file_path_for_signage) is False:
            frame_clean(output_frame)
            ttk.Label(output_frame, text="Incorrect file for signage path!", font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
            return
        
        elif path_valid(file_path_to_verify) is False:
            frame_clean(output_frame)
            ttk.Label(output_frame, text="Incorrect file to verify path!", font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
            return
        
        elif path_valid(file_path_to_verify_signature) is False:
            frame_clean(output_frame)
            ttk.Label(output_frame, text="Incorrect signature file path!", font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
            return
        ...
        # RSA Crypto incorrect password error handler
        frame_clean(output_frame)
        ttk.Label(output_frame, text=f"Incorrect password! Please try again.", font=("Consolas", 12, "bold"), bootstyle='danger').pack(pady=10)
...
def gui_window_start(cert_auth):
    ...
    ttk.Button(file_frame, text='VIEW FILE', command=lambda: select_file(file_to_be_signed_entry), bootstyle='info-outline').place(x=710, y=500, width=100, height=40)
    ...
    ttk.Button(file_frame, text='VIEW FILE', command=lambda: select_file(file_to_be_verified), bootstyle='info-outline').place(x=710, y=590, width=100, height=40)
    ...
    ttk.Button(file_frame, text='VIEW FILE', command=lambda: select_file(file_with_separate_signature), bootstyle='info-outline').place(x=710, y=680, width=100, height=40)

    # SUBMIT Button
    ttk.Button(file_frame, text='SUBMIT', command=lambda: signature_on_submit(cert_auth, output_frame, username_entry, password_entry, file_to_be_signed_entry, file_to_be_verified, file_with_separate_signature)).place(x=450, y=750, width=100, height=40)
